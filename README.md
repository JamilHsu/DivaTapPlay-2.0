# Tap Play on *Hatsune Mike Project Diva Mega Mix+* (PC)!

![image](https://raw.githubusercontent.com/JamilHsu/DivaTapPlay/refs/heads/master/%E2%96%B3%E2%96%A1%C3%97%E2%97%AF.jpg)
![image](https://raw.githubusercontent.com/JamilHsu/DivaTapPlay/refs/heads/master/XYBA.jpg)

請務必查看`DivaTapPlaySettings.txt`以確認有哪些設定可以調整。

由於我無法偵測遊戲何時在進行節奏遊戲中，我目前的設計是透過檢測滑鼠游標是否已被隱藏來自動切換啟用/停用狀態。當然，這個檢測方法並不準確，因此你可能需要手動切換啟用/停用狀態。  
切換的方法是透過一隻手指從頂端滑到底部。透過手勢來停用的話，不會自動再次啟用(或者換句話說，只有是自動停用時才會自動啟用)

Please check `DivaTapPlaySettings.txt` to see which settings can be adjusted.
